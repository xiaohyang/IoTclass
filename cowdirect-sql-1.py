...
if __name__ == "__main__":
    ...
    re.sub(r"\s+", "", x, flags=re.UNICODE)).reduce(add)
      ...
      print("JSON Schema\n=====")
            ...
            # Search
            sqlContext.sql("select payload.data.temperature from iotmsgsTable where payload.data.temperature = 39").show(n=100)

            # Filter
            sqlContext.sql("select payload.data.temperature from iotmsgsTable where payload.data.temperature > 39.5").show(n=100)
