...
if __name__ == "__main__":
    ...
    def process(time, rdd):
	    ...
	    print("JSON Schema\n=====")
            ...
            # Sort
            sqlContext.sql("select * from iotmsgsTable order by temperature desc").show(n=100)

            # Filter
            sqlContext.sql("select payload.data.ID, payload.data.temperature from iotmsgsTable where payload.data.temperature > 39.5").show(n=100)

            
            #########################################
            # detect low rumination and high variability
            ########################################

            # Sort
            sqlContext.sql("select * from iotmsgsTable order by rumination asc").show(n=100)

            # Filter
            sqlContext.sql("select payload.data.ID, payload.data.rumination from iotmsgsTable where payload.data.rumination < 400.0").show(n=100)

            # Calucate
            #std = sqlContext.sql("select STDEVP(payload.data.rumination) 
            #if std > 80
